...
public final class Game {
    ...
    private void playAgentTurn(int round, Player player) {
        ...
        Action chosen = player.getStrategy().select(legalActions, player, state);
        if (chosen == null) {
    ...
    private void playHumanTurn(int round, Player player) {
        ...
        while (true) {
            System.out.println("Enter command: Roll | Go | List | Undo | Redo | Build settlement <nodeId> | Build city <nodeId> | Build road [from,to]");            String input = scanner.nextLine();
            ...
            switch (cmd.getType()) {
                ...
                case GO -> {
                    executeRecordedAction(round, player, new PassAction());                    stateExporter.export(state);
                    return;
                }
